...
public:
    ...
    void solveNQueensUtil(int col, vector<int>& leftRow, vector<int>& leftDia, vector<int>& rightDia, vector<string> board) {
        if(col == board[0].size()) {
            ans.push_back(board);
            return;
        }
        ...
        for(int row = 0;row < board.size();row++) {
            if(!leftRow[row] && !leftDia[board.size() - 1 + row - col] && !rightDia[row + col]) {
                ...
                solveNQueensUtil(col + 1, leftRow, leftDia, rightDia, board);
    ...
    vector<vector<string>> solveNQueens(int n) {
        ...
        solveNQueensUtil(0, leftRow, leftDia, rightDia, board);
        return ans;
    }
};